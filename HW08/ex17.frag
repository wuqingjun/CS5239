...
void main(void)
{
   ...
   if(mode == 7){
       gl_FragColor = length(gl_Color.rgb) * texture2D(bamBoobTexture, gl_TexCoord[0].st);
   }
   else if(mode == 8){
       float n = snoise(v_texCoord3D * 6.0);
       gl_FragColor = length(gl_Color.rgb) * texture2D(bamBoobTexture, gl_TexCoord[0].st) * vec4(0.5 + 0.5 * vec3(n, n, n), 1.0);
   }
   else if(mode == 9){
       ...
       gl_FragColor = length(gl_Color.rgb) * texture2D(bamBoobTexture, gl_TexCoord[0].st) * vec4(vec3(1.0, 0.5, 0.0) + vec3(n, n, n), 1.0);
   }
   else {
